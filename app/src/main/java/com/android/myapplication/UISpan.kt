...
sealed class UISpan {
    ...
    class UITextColorSpan(@ColorRes private val colorRes: Int) : UISpan() {
        override fun asSpan(context: Context): Any {
            return ForegroundColorSpan(ContextCompat.getColor(context, colorRes))
        }
    }

    class UIBackgroundColorSpan(@ColorRes private val colorRes: Int) : UISpan() {
        override fun asSpan(context: Context): Any {
            return BackgroundColorSpan(ContextCompat.getColor(context, colorRes))
        }
    }

    class UIRelativeSizeSpan(private val factor: Float) : UISpan() {
        override fun asSpan(context: Context): Any {
            return RelativeSizeSpan(factor)
        }
    }

    class UIClickableSpan(private val onClick: () -> Unit) : UISpan() {
        override fun asSpan(context: Context): Any {
